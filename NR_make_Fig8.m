...
%
% Calls             NR_eisim, NR_calc_z_spectral, NR_sinusoids_to_cmodel_images
...
ylabel( 'Vm [mV]' )
xlabel( 'Frequency [Hz]' );
tstr                            = sprintf( 'MDL=%d; Ain=%0.3g; Iapp=%0.3g; Dn=%0.3g' ...
    , MDL, MDL_params( 1 ), MDL_params( 2 ), MDL_params( 4 ) );
title( tstr )
...
[ cohI, phsI, foI, ~, fidxI ]   = st_coherence( st, Ie, 'spkFs', Fs, 'xFs', Fs ...
    , 'fROI', fROI, 'Fs', aFs, 'graphics', graphics0 ...
    , 'M', M, 'mtNW', mtNW, 'nFFT', nFFT, 'dflag', dflag );
s.cohs                          = cohI;
s.fidx                          = fidxI;
s.fo                            = foI;
s.phs                           = phsI;
...
Fs                              = 1 / diff( t( 1 : 2 ) ) * 1000;
[ cohI, phsI, foI, ~, fidxI ]   = st_coherence( st, Ie, 'spkFs', Fs, 'xFs', Fs ...
    , 'fROI', fROI, 'Fs', aFs, 'graphics', graphics0, 'M', M ...
...
if reCompute
    ...
    for i                       = 1 : nAin
        for j                   = 1 : nreps
            ...
            [ t, ~, Vi_1, ~ ...
                , finput_i ]    = NR_eisim( { MDL, [ Ain( i ), Iapp NaN D_i ] } );
            stimes              = local_parse( find( Vi_1 > 0 ) );
            if ~isempty( stimes )
...
for i                           = 1 : length( MDLs )
    ...
    for rc                      = 0 : 1
        ...
        Fs                      = 1 / diff( t( 1 : 2 ) ) * 1000;
        [ cohI, phsI, foI ...
            , ~, fidxI ]        = st_coherence( st, Ie, 'spkFs', Fs ...
            , 'xFs', Fs , 'fROI', fROI, 'Fs', aFs, 'graphics', graphics0 ...
            , 'M', M, 'mtNW', mtNW, 'nFFT', nFFT, 'dflag', dflag );
        s.cohs                  = cohI;
        s.fidx                  = fidxI;
        s.fo                    = foI;
